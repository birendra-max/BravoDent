...
echo "<h3 class='text-center btn btn-warning'> All Redesign Cases</h3>";
  ?>
                    ...
                    <table id="example1" class="table table-bordered table-striped">
                       <thead>
                        ...
                        <tbody>
                          ...
                          $i=0;
                          $tdate=date('d-M-Y');
                       $sql="SELECT * FROM orders where status_r='Rush'";
                        $res=mysqli_query($bd,$sql);
                        while($row=mysqli_fetch_array($res))
                        {
                          // if (strtotime($sdate)<=strtotime(date("d-M-Y",strtotime($row["created_at"]))) and strtotime(date("d-M-Y",strtotime($row["created_at"]))) <=strtotime($edate)) {
                       ...
                       $msgrow=mysqli_fetch_array($resmsg);
                            ...
                            <td><?php $eee=$row['c_id'];
                            $sqlnn="SELECT name FROM user1 where em='$eee'";
                        $reennn=mysqli_query($bd,$sqlnn);
                            
                            $rownn=mysqli_fetch_array($reennn);
                            echo $rownn['name'] ?></td>
                           </tr>
                           <?php
                          //}
